...
| Every release opens every container earlier releases wrote — checked against files written by the released 0.2.0, 0.3.0 and 0.4.0 binaries, fetched from Maven Central. | Semantic, not byte-level: ZIP metadata differs between saves. And forwards only: an older release refuses a `container_version` it does not know, by design. |
